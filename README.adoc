...
= Burble
image:https://img.shields.io/badge/OpenSSF-Best_Practices-green?logo=openssourcesecurity[OpenSSF Best Practices,link="https://www.bestpractices.dev/en/projects/new?repo_url=https://github.com/hyperpolymath/burble"]
...
image::brand/social-preview.svg[Burble,align="center"]
...
* *Test gate:* the full server suite (~707 tests) carries known failures and
  the CI pass/fail gate is temporarily disarmed while they are triaged
  (issue #100). A green *gated* run is the criterion for CRG grade C;
  until then the self-assessed grade is D
  (see `.machine_readable/6a2/STATE.a2ml`).
...
pure Elixir for LZ4 compression, 62x for echo cancellation, 37x for FFT —
*as library microbenchmarks*. The live call path forwards opaque Opus and does
not currently traverse these kernels (they serve self-test and archive paths);
end-to-end numbers are tracked in issue #52.
...
SIMD-accelerated audio processing via Zig NIFs. *Honest framing (ADR-0007):
these are library microbenchmarks of the kernels in isolation — the live SFU
path forwards opaque Opus RTP and does not execute them. End-to-end
mic-to-speaker latency is not yet benchmarked (issue #52).*
