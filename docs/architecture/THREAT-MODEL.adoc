...
Burble is a voice-first, self-hostable, end-to-end encrypted communications platform designed for teams, communities, and embedded applications. The system supports four deployment topologies—monarchic (single-server), oligarchic (federated cluster), distributed (open mesh), and serverless (peer-to-peer)—each balancing scalability, moderation, and decentralization. The control plane runs on Elixir/Erlang OTP with per-room GenServer isolation, ensuring fault tolerance and sub-millisecond signaling. The media plane uses a WebRTC-compatible SFU (Selective Forwarding Unit) that *does not* decrypt media: voice frames pass through uninterpreted for end-to-end encryption. Protocol bridges (SIP, Mumble) transcode messages at the control plane only, preserving the trust boundary that the SFU neither sees nor modifies encrypted audio. Audio processing (compression, echo cancellation, denoising) runs as Zig SIMD-compiled NIFs invoked by Elixir, achieving 26,350x speedup over byte-loop implementations. Dependent-type proofs in Idris2 formally verify the ABI surface and media pipeline linearity.
...
NOTE: Items marked `(optional)` reference tools (svalinn, cerro-torre, vordr) that are referenced in the Burble Trustfile but are not yet installed in this repository as of 2026-05-07. See `docs/superpowers/specs/2026-05-07-burble-full-implementation-scoping.md` §7 R6.
