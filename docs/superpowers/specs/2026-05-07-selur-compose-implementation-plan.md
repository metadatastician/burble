...
| P-2 | Create `github.com/hyperpolymath/selur-compose` from `rsr-template-repo`. Apply the org-standard files (`0-AI-MANIFEST.a2ml`, `LICENSE` MPL-2.0, `CODE_OF_CONDUCT.md`, `CONTRIBUTING.md`, `SECURITY.md`, `EXPLAINME.adoc`, `MAINTAINERS.adoc`). | [I] | S | P-1 | Repo exists, default branch `main`, branch protection on. |
| P-3 | Author workspace `Cargo.toml` declaring members `crates/selur-compose-schema`, `crates/selur-compose-interp`, `crates/selur-compose-plan`, `crates/selur-compose-driver`, `crates/selur-compose`. Set `[workspace.package]` with `version = "0.1.0"`, `edition = "2021"`, `rust-version = "1.78"`, `license = "MPL-2.0"`, `repository`, `homepage`. Add `[workspace.dependencies]` block pinning `serde`, `serde_json`, `toml = "0.8"`, `thiserror`, `anyhow`, `tokio`, `clap`, `tracing`, `tracing-subscriber`, `humantime`, `humantime-serde`, `sha2`, `async-trait`, `futures`, `insta`, `proptest`, `rstest`, `assert_cmd`, `predicates`. | [A] | S | P-2 | `cargo metadata` succeeds; resolver `2`. |
